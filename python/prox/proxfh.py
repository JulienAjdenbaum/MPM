...
def prox(y, k, X, convo, a, b, D, x, mu, eps):
    ...
    grad = fftconvolve(b ** 2 * convo + b * a - b * y, X[::-1, ::-1, ::-1], "same") * (a + b * convo)
    forward = k - alph * grad
...
# def mylambertw(x):
#     utils.myprint("deb lambert", np.max(x), np.min(x))
#     utils.myprint("deb lambert", np.max(np.exp(x)), np.min(np.exp(x)))
#     v = np.where(x < 1e2, Lambert_W(x), x - np.log(np.maximum(x, 1e-10)))
#     utils.myprint("fin lambert", np.max(v), np.min(v))
#     return v

def w(nu, h, c, lamb, gam):
    # utils.myprint("w :", np.max(c), np.max(k), nu)
...
def nu_hat(lamb, gam, h, c):
    ...
    while True:
        ...
        if np.abs(newNu) > 1e50 or np.isnan(nu[-1]) or newNu < -1e50:
            ...
            nu[-1] = newNu
            # utils.myprint("Nan : reset nu", nu, nIter)
            # print(k)
            utils.myprint("max k, min k", np.max(h), np.min(h))
            utils.myprint("max c, min c", np.max(c), np.min(c))
            raise Exception('nu is nan :(')
        if nIter > maxIter or np.abs(nu[-1] - nu[-2]) < epsilon:
            break
    utils.myprint("nu = ", nu)
    utils.myprint("pĥi(nu)", phi(nu[-1], h, c, lamb, gam))
    return nu


def phi(nu, k, c, lamb, gam):
    utils.myprint("lamb phi", np.max(w(nu, k, c, lamb, gam)))
    W = ProxEntropyNu(k, gam, lamb, c, nu)
    return lamb * gam * np.sum(W) - 1


def dphi(nu, k, c, lamb, gam):
    utils.myprint(nu, lamb, gam)
    W = ProxEntropyNu(k, gam, lamb, c, nu)
    utils.myprint("W dphi", np.max(W), "  ", np.min(W))
    utils.myprint("retour dphi", -np.sum(W / (1 + W)))
...
def ProxEntropyNu(k, gam, lam, c, nu):
    ...
    utils.myprint("U2 ", np.max(U), np.min(U))
    return U
